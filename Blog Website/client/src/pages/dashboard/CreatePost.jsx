...
import { Save, Send, Image as ImageIcon, X, Layout, Eye, Type, Settings, Hash, MoreHorizontal, ChevronLeft, Loader2, Sparkles, PenTool } from 'lucide-react';
...
const CreatePost = () => {
    ...
    const onSubmit = async (e, status = 'draft') => {
        if (e) e.preventDefault();
        if (!title.trim()) return toast.error('Metadata incomplete: Title required');
        ...
        try {
            ...
            toast.success(status === 'submitted' ? 'Artifact dispatched for review' : 'Draft secured in archives');
            navigate('/dashboard');
        } catch (error) {
    ...
    return (
        <div className="max-w-[1440px] mx-auto space-y-10 pb-32 animate-fade-in px-6">
            {/* 1. Cinematic Action Command Bar */}
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 sticky top-24 z-50 bg-white/90 backdrop-blur-xl p-8 rounded-xl border border-slate-100 shadow-2xl shadow-slate-200/40 translate-y-2">
                <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-1">
                        <h1 className="text-2xl md:text-3xl font-display font-black text-slate-900 tracking-tight flex items-center gap-3">
                            Initiate Broadcast <Sparkles className="h-5 w-5 text-primary-500 animate-pulse" />
                        </h1>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Status: Drafting Archive Artifact</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    ...
                    >
                        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Save className="mr-2 h-4 w-4" /> Secure Draft</>}
                    </button>
                    <button
                    ...
                    >
                        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Send className="mr-2 h-4 w-4" /> Dispatch Artifact</>}
                    </button>
                </div>
            </header>

            <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 pt-4">
                {/* 2. Primary Composition Terminal */}
                            ...
                            >
                                <div className="bg-white p-12 md:p-20 rounded-xl border border-slate-100 shadow-2xl shadow-slate-200/40 space-y-12 min-h-[900px]">
                                    <input
                                        ...
                                        value={title}
                                        onChange={onChange}
                                        placeholder="Headline your technical artifact..."
                                    ...
                                    <textarea
                                        ...
                                        value={content}
                                        onChange={onChange}
                                        placeholder="Articulate your narrative journey here..."
                            ...
                            >
                                <div className="max-w-4xl mx-auto">
                                    <div className="mb-10 text-center space-y-4">
                                        <div className="inline-block px-4 py-1.5 bg-primary-100 text-primary-700 rounded-full text-[10px] font-black uppercase tracking-widest">Global Transmission Preview</div>
                ...
                <aside className="lg:col-span-1 space-y-8">
                    <div className="bg-white p-8 rounded-xl border border-slate-100 shadow-xl shadow-slate-200/40 space-y-10 sticky top-64 transition-all">
                        <div className="flex items-center gap-3 border-b border-slate-50 pb-6">
                            ...
                            <h3 className="font-black text-slate-900 uppercase tracking-widest text-[11px]">Artifact Metadata</h3>
                        </div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="space-y-3">
                                <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">
                                    <Type className="h-3 w-3" /> Executive Synopsis
                                </label>
                                <textarea
                                    id="input-excerpt"
                                    name="excerpt"
                                    ...
                                    rows="5"
                                    placeholder="Brief hook for central indices..."
                                ></textarea>
                            </div>
                        ...
                        <div className="pt-8 border-t border-slate-50 text-center">
                            <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.3em] leading-relaxed">Reading latency is calculated via <br /> central processing engine</p>
                        </div>
                    </div>
                </aside>
            </div>
